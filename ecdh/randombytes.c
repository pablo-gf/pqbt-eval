...
#if defined(STM32F2) || defined(STM32F4) || defined(STM32L4R5ZI)
...
#else /* NONRANDOM FALLBACK IMPLEMENTATION (for STM32F3, etc.) */
#warning Using a non-random randombytes (deterministic fallback for benchmarking)
...
static void randombytes_regen(void)
{
  uint32_t out[8];
  if (!++in[0]) {
    if (!++in[1]) {
